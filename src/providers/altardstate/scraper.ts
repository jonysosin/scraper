...
const scraper: IScraper = async (request, page) => {
  ...
  const { protocol, resource, pathname, query: { pid } } = parseUrl(request.pageUrl)

  const baseProductUrl = `${protocol}://${resource}${pathname}?pid=${pid}`
  const baseProductData = await getProductJson(page, baseProductUrl)

  // Website screenshot
  // console.log(`${protocol}://${resource}${baseProductData.product.selectedProductUrl.split('?')[0]}`)
  await page.goto(`${protocol}://${resource}${baseProductData.product.selectedProductUrl.split('?')[0]}`)
  ...
  const additionalSections = await page.evaluate((DESCRIPTION_PLACEMENT) => {
    ...
    document.querySelectorAll('.pdp-description-details .accordion-wrapper').forEach(element => {
      const name = (element.querySelector('.pdp-link-desc-detail')?.textContent || '').replaceAll('\n', '').trim()
      const content = (element.querySelector('.more-info-body')?.outerHTML || '').replaceAll('\n', '').trim()
  ...
  // Size chart html
  const sizeChartHtml = await getSelectorOuterHtml(page, '#openSizeGuideModal > div > div > div.modal-body > div')
  ...
  for (const variantData of variantsData) {
    ...
    const variantSize = productData.product.variationAttributes.find(x => x.attributeId === 'size')?.values.find(x => !!x.selected)
    const variantColor = productData.product.variationAttributes.find(x => x.attributeId === 'color')?.values.find(x => !!x.selected)
    ...
    product.availability = productData.product.available && variantSize?.selectable && variantColor?.selectable
...
const getProductVariantUrls = (baseProductData: TProviderData, baseProductUrl: string, productId: string) => {
  const variants = baseProductData.product.variationAttributes.map(({ attributeId, values }) => values.map(x => `${attributeId}:${x.value}`));
